          ...
          <div className="r-grid-3" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'22px' }}>
            {[
              ...
              <div key={t.name} className="tier-card" style={{ border:`1px solid ${t.featured ? 'rgba(201,169,110,.4)' : 'var(--border)'}`, background:t.featured ? 'var(--bg3)' : 'var(--bg2)', padding:'44px 36px', position:'relative' }}>
                ...
                <div style={{ marginBottom:'16px', padding:'12px 14px', background:'rgba(201,169,110,.06)', border:'1px solid rgba(201,169,110,.2)' }}>
                  <div style={{ display:'flex', alignItems:'baseline', gap:'4px', marginBottom:'4px' }}>
                    <span style={{ fontFamily:'Cormorant Garant,serif', fontSize:'1rem', color:'var(--gold)' }}>₺</span>
                    <span style={{ fontFamily:'Cormorant Garant,serif', fontSize:'2.2rem', fontWeight:300, color:'var(--cream)', lineHeight:1 }}>0</span>
                    <span style={{ fontFamily:'Cormorant Garant,serif', fontSize:'.78rem', color:'var(--muted)' }}>/ay</span>
                  </div>
                  <div style={{ fontFamily:'Cormorant Garant,serif', fontSize:'.75rem', color:'var(--gold-d)', lineHeight:1.55, marginBottom:'4px' }}>
                    Danışan kabulü başladıktan sonra 6 ay ücretsiz
                  </div>
                  <div style={{ fontFamily:'Cormorant Garant,serif', fontSize:'.72rem', color:'var(--muted)' }}>Sonrası: {t.price}/ay</div>
                </div>
